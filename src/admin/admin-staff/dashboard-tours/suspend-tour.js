...
function SuspendTour(){
    ...
    useEffect(() => {
        // Hàm để lấy và lọc các tour không có lịch trình hoặc có lịch trình < ngày hiện tại
        const filterToursWithoutUpcomingSchedules = async () => {
          ...
          for (const tour of tours) {
            try {
              ...
              // Kiểm tra nếu tour không có lịch trình hoặc tất cả lịch trình < ngày hiện tại
              const pastSchedules = schedules.filter((schedule) => {
                const tourDate = new Date(schedule.day_depart);
                return tourDate < today; // Ngày khởi hành < ngày hiện tại
              });
      
              // Nếu tour không có lịch trình hoặc tất cả lịch trình đều < ngày hiện tại, thêm vào danh sách kết quả
              if (schedules.length === 0 || pastSchedules.length === schedules.length) {
                filteredTours.push({
                  ...tour,
                  schedules: pastSchedules, // Lưu các lịch trình đã qua (nếu có)
                });
              }
            } catch (error) {
          ...
          setUpcomingTours(filteredTours); // Lưu các tour không có lịch trình hoặc đã qua ngày hiện tại
        };
      
        if (tours.length > 0) {
          filterToursWithoutUpcomingSchedules();
        }
      }, [tours]);
