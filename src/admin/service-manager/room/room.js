...
import { fetchRoomImages } from "../../../component/api/room";
...
function RoomManager() {
    ...
    const [idRoom, setIdRoom] = useState(idRoomValue);
    const [selectedImage, setSelectedImage] = useState(null); // Lưu hình ảnh được chọn
    const [formImage, setFormImage] = useState(initFormImage);
    ...
    const getDataImage = async (roomId) => {
        
        try {
            // Gọi API để lấy danh sách Facilities
            const ImageResponse = await fetchRoomImages(roomId);
    ...
    // update room thumb
    const updateRoomThumb = (roomId, imageId) => {
        // console.log(roomId, imageId);
        // if (window.confirm('Bạn có chắc chắn muốn xóa phòng này?')) {
            fetch('http://localhost:88/api_travel/api/admin/update-room-thumb.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const deleteRoomImage = (imageId) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa phòng hình ảnh này?')) {
            fetch('http://localhost:88/api_travel/api/admin/delete_room_image.php', {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ room_image_id: imageId }),
            })
                .then(response => response.json())
                .then(data => {
                    if (data.status === 'success') {
                        setRoomImages(roomImages.filter(image => image.id !== imageId));
                        toast.success(data.message);
                    } else if (data.status === 'error') {
                        toast.error(data.message);
                    } 
    ...
    const handleModalImage = (room) => {
        setIdRoom(room.id);
        console.log(room.id);
        setIsOpenModalImage(!isOpenModalImage);
        getDataImage(room.id);
    };
    ...
    const hendleImageSubmit = async (event) => {
        ...
        // initFormSchedule.forEach((tour, index) => {
            image.append("room_id", idRoom);
            image.append("image", formImage.image); // File sẽ được gửi dưới dạng multipart
        // });
            console.log("formDataImage", idRoom);
        fetch('http://localhost:88/api_travel/api/admin/create_room_image.php', {
            method: 'POST',
            // headers: {
            //   'Content-Type': 'application/json',
            // },
          ...
          .then(data => {
            if (data.status === 'success') {
                toast.success(data.message);
                setFormImage(initFormImage); // Đặt lại form về rỗng nếu có lỗi
            } else if (data.status === 'error'){
                toast.error(data.message);
                setFormImage(initFormImage); // Đặt lại form về rỗng nếu có lỗi
            } 
                                                    ...
                                                    <td>
                                                        ...
                                                        <div className="flex justify-center gap-2">
                                                            ...
                                                            <div className="bg-[#0dcaf0] px-2 rounded-md">
                                                                <button type='button' onClick={() => handleModalImage(room)} className='py-1 font-semibold text-sm'><i className="fa-solid fa-image"></i></button>
                                                            </div>
                    ...
                    <div className="modal-dialog modal-lg w-full">
                        <div className="modal-content w-full px-2">
                            ...
                            {Array.isArray(roomImages) && roomImages.length > 0 ? (
                                <h5 className="modal-title font-medium text-xl">{roomImages[0].room_name}</h5>
                            ) : (
                                ...
                                >
                                    ...
                                    <table className="w-full max-w-full mb-4 bg-transparent table-hover border text-center">
                                        ...
                                        <tbody id="room-image-data" key={image.id}>
                                            <tr className="">
                                                <td><img src={`http://localhost:88/api_travel/api/Images/room/${image.image}`} className='w-full h-[300px] object-cover' alt="" /></td>
                                                <td><input type="checkbox"
                                                        ...
                                                        onClick={() => updateRoomThumb(image.room_id, image.id)}
                                                        />
                                                </td>
                                                <td>
